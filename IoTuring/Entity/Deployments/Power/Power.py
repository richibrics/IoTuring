...
class Power(Entity):
    ...
    def PostInitialize(self):
        ...
        if self.os in commands_shutdown:
            self.RegisterEntityCommand(EntityCommand(self,KEY_SHUTDOWN,self.CallbackShutdown))
        
        # Reboot
        if self.os in commands_reboot:
            self.RegisterEntityCommand(EntityCommand(self,KEY_REBOOT,self.CallbackReboot))
        ...
        if lookup_key in commands_sleep:
            ...
            self.RegisterEntityCommand(EntityCommand(self,KEY_SLEEP,self.CallbackSleep))

        
    def CallbackShutdown(self, message):
        subprocess.Popen(commands_shutdown[self.os].split(), stdout=subprocess.PIPE)

    def CallbackReboot(self, message):
        subprocess.Popen(commands_reboot[self.os].split(), stdout=subprocess.PIPE)
